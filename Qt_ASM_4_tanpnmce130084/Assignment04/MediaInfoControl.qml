...
Item {
    ...
    PathView {
        ...
        preferredHighlightBegin: 0.5    // Set album art is focused to middle
        preferredHighlightEnd: 0.5      // Set album art is focused to middle
        focus: true
        model: myModel
        delegate: appDelegate
        pathItemCount: 3    // Up to 3 album art displayed on screen at a time
        path: Path {
            startX: 65
            startY: 50
    ...
    ButtonControl {
        ...
        onClicked: {
            ...
            (player.playlist.currentIndex === 0) ? player.playlist.setCurrentIndex(myModel.rowCount() - 1) : player.playlist.previous()
        }
    }
    ButtonControl {
        id: play
    ...
    ButtonControl {
        ...
        onClicked: {
            ...
            (player.playlist.currentIndex === -1) ? player.playlist.setCurrentIndex(0) : player.playlist.next()
        }
    }
    SwitchButton {
        id: repeater
    ...
    Connections{
        ...
        onCurrentIndexChanged: {
            ...
            // set index of album art to in index of first song and stop player
            if(index === -1) {
                album_art_view.currentIndex = 0
                player.stop()
            } else album_art_view.currentIndex = index; // Otherwise, set current index of album art by index
        }
    }
}
